...
def master_sparse_recon(image_paths, start_index, end_index, model, known_focal, refine_pose=True, number_match=10000):
    ...
    if refine_pose:
        ...
        return {"poses": [poses[0], opt_c2w], 
                ...
                "matches": [updated_matches_im0, high_conf_matched_2d_2],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_views', type=int, default=64, help="Number of views to process")
    ...
    poses = to_numpy(scene.get_im_poses())
    global_aligned_poses = poses @ np.linalg.inv(poses[0])

    pts3d = to_numpy(scene.get_pts3d())

    global_aligned_pts = [transform_pc_to_cam_coord(pc.reshape(-1, 3), extri) for extri, pc in zip(poses, pts3d)]
    ...
    for i in range(1, math.ceil(args.n_views / args.fragment_size) + 1):
        ...
        anchor_3d_all_cam = global_aligned_pts[anchor_idx]
        anchor_3d_all = anchor_3d_all_cam.reshape(low_size[1], low_size[0], 3)
        anchor_depth = get_depth_from_3d_points(anchor_3d_all_cam, np.eye(4)).reshape(anchor_3d_all.shape[0], anchor_3d_all.shape[1])
        ...
        for corres_dict in local_all_corres_dict:
            ...
            matched_anchor_3d = unproject_2d_to_3d(corres_dict["matches"][0], corres_dict["poses"][0], corres_dict["intrin"][0], depths=anchor_depth_values)
            matched_pair_3d = unproject_2d_to_3d(corres_dict["matches"][0], corres_dict["poses"][0], corres_dict["intrin"][0], depths=pair_depth_value)
    ...
    for mm in range(len(all_windows)-1):
        prev_window = all_windows[mm]
        next_window = all_windows[mm+1]
        start_idx=mm*args.fragment_size
        end_idx=(mm+1)*args.fragment_size
        next_anchor_idx =  np.where(global_indices == end_idx)[0][0]
        view_4_world_pose = global_aligned_poses[next_anchor_idx]
        all_poses.append(view_4_world_pose)
        

        for corres_dict in next_window: 
            current_pose = corres_dict["poses"][1]
            init_pose = view_4_world_pose @ current_pose
            all_poses.append(init_pose)
            corres_dict_3d_world = transform_point_cloud(corres_dict["3d_points"], view_4_world_pose)
            corres_dict["3d_points"] =corres_dict_3d_world
