...
run_on_gpu() {
    ...
    CUDA_VISIBLE_DEVICES=${GPU_ID} python ./sparse_co3d.py \
    -s ${SOURCE_PATH} \
    --n_views ${N_VIEW}
 
    # (2) Hierarchical 3D Gaussian Alignment
    CUDA_VISIBLE_DEVICES=${GPU_ID} python ./train_co3d.py \
    ...
    # (3) Render Test Views
    CUDA_VISIBLE_DEVICES=${GPU_ID} python ../render_index_new.py \
    -s ${SOURCE_PATH} \
    -m ${MODEL_PATH} \
    --n_views ${N_VIEW}  \
    --scene ${SCENE} \
